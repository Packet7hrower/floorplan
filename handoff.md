...
## 2026-07-25 18:06 CDT — Production branch merge and GHCR deployment

GitHub identifies `main`, not `master`, as the repository default branch. The completed application changes were committed on `codex/ux-roadmap` as `e7e6fdb5b6cb0461b386a6e4536e38d550a7a0c9`, pushed to the matching remote branch, and merged with an explicit merge commit into `main` as `f5d5e7c588498d6c06b633619f275f93acb438d5`. Remote refs were verified directly after publication.

Main-branch workflow `30178469470` completed successfully. Its `test` job passed `npm ci`, lint, 55 Vitest tests, the production Vite build, browser installation, and all 33 Chromium/Firefox/WebKit acceptance journeys. Its `publish` job produced and pushed the public Linux AMD64/ARM64 OCI image index `ghcr.io/packet7hrower/floorplan:latest` with digest `sha256:3f0da0b0b91946e8beaa759c9c4a5801762d3d6e6fac3fad4eb98d468217fe22`. The final `runtime-smoke` job validated the root Compose file, pulled the image anonymously, started it, verified `/healthz`, SPA fallback, and unprivileged execution, and stopped the temporary stack cleanly.

The only workflow annotation is the previously documented third-party GitHub Actions Node 20 runtime deprecation; GitHub executed those actions on Node 24 and every job passed. This release updates the GitHub `main` source and public GHCR image only. It does not create or replace a persistent Home Lab deployment, select a Docker host, consume a host port, or alter DNS/networking.
