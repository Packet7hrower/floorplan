import { Box, Copy, Files, Info, PanelRightClose, RotateCw, X } from "lucide-react";
...
function formatCoordinate(coordinate: number, unit: Unit): string {
  return `${coordinate < 0 ? "−" : ""}${formatMeasurement(Math.abs(coordinate), unit)}`;
}

function CoordinateField({ label, value, unit, onCommit }: MeasurementFieldProps) {
  const [input, setInput] = useState(value === 0 ? "0" : formatCoordinate(value, unit));
  const [error, setError] = useState<string | null>(null);
  useEffect(() => setInput(value === 0 ? "0" : formatCoordinate(value, unit)), [value, unit]);
  const commit = () => {
    try {
      const normalized = input.trim().replace(/^−/, "-");
      const parsed = normalized === "0" ? 0 : normalized.startsWith("-")
        ? -parseMeasurement(normalized.slice(1), unit)
        : parseMeasurement(normalized, unit);
      onCommit(parsed);
      setError(null);
    } catch (reason) {
      setError(reason instanceof Error ? reason.message : "Invalid coordinate.");
    }
  };
  return (
    <label className={"field " + (error ? "invalid" : "")}>
...
export function PropertiesPanel({ roomReady }: PropertiesPanelProps) {
  ...
  const setRightRailCollapsed = useUiStore((value) => value.setRightRailCollapsed);
  ...
  const breadcrumb = selectedWall
    ...
    : selectedOpening
      ? `Project / ${selectedOpening.kind === "door" ? "Door" : "Window"}`
      : selectedFurniture
        ? `Project / ${FURNITURE_CATALOG[selectedFurniture.catalogType].label}`
        : "Project / Room";

  return (
    <aside className="properties-panel" aria-label="Properties">
      <div className="panel-heading properties-heading">
        <span className="selection-breadcrumb">{breadcrumb}</span>
        <span className="panel-heading-actions">
          {state.selection && <button type="button" className="icon-button" aria-label="Clear selection" onClick={() => state.setSelection(null)}><X size={15} /></button>}
          <button type="button" className="icon-button" aria-label="Collapse properties" onClick={() => setRightRailCollapsed(true)}><PanelRightClose size={15} /></button>
        </span>
      </div>
      {selectedWall ? (
        <>
        ...
        <>
          ...
          <div className="field-grid">
            <CoordinateField label="Position X" value={selectedFurniture.x} unit={state.project.displayUnit} onCommit={(x) => state.updateFurniture(selectedFurniture.id, { x })} />
            <CoordinateField label="Position Y" value={selectedFurniture.y} unit={state.project.displayUnit} onCommit={(y) => state.updateFurniture(selectedFurniture.id, { y })} />
          </div>
          ...
          <div className="selection-command-grid">
            <button type="button" className="button secondary" onClick={state.copySelectedFurniture}><Copy size={15} />Copy</button>
            <button type="button" className="button secondary" onClick={state.duplicateSelectedFurniture}><Files size={15} />Duplicate</button>
            <button type="button" className="button secondary" onClick={state.rotateSelectedFurniture90}><RotateCw size={15} />Rotate 90°</button>
          </div>
          {state.view === "3d" && <button type="button" className="button primary" onClick={() => state.setView("2d")}><Box size={16} />Edit in 2D</button>}
